...
class CalculoProduto:
    def __init__(self, janela_menu):
        ...
        self.root.title("Calculo do produto")
    ...
    def atualizar_produto(self):
        ...
        if operacao == "Subtrair":
            ...
            for resultado in resultados:
                ...
                if valor_restante <= quantidade_estoque:
                    ...
                    atualizacoes.append((id_produto, nf, nova_quantidade_estoque))
            ...
            # Ajustamos a query para pegar primeiro as NFs mais recentes (operação inversa da subtração)
            query = """
            SELECT sp.id, sp.nf, sp.produto, sp.peso_liquido, COALESCE(eq.quantidade_estoque, sp.peso_liquido)
            FROM somar_produtos sp
            LEFT JOIN estoque_quantidade eq ON sp.id = eq.id_produto
        ...
        for id_produto, nf, nova_quantidade in atualizacoes:
            insert_query = """
        ...
        for id_produto, nf, nova_quantidade in atualizacoes:
            for item in self.treeview.get_children():
                ...
                if valores[1] == nf:  # Comparação pelo número da NF
                    try:
                        quantidade_decimal = Decimal(valores[3].replace('.', '').replace(',', '.').strip())
                    except InvalidOperation:
                        messagebox.showerror("Erro", f"Valor inválido no campo quantidade: {valores[3]}")
                        cursor.close()
                        conn.close()
                        return
                    ...
                    elif nova_quantidade != quantidade_decimal:
                        self.treeview.item(item, tags="partial_red_text")
                    else:
                        self.treeview.item(item, tags="")

                    # Atualiza os valores (coluna 3 é a quantidade original, coluna 4 é a atualizada)
                    self.treeview.item(
                        item,
                        values=(valores[0], valores[1], valores[2], valores[3], quantidade_formatada) + valores[5:]
                    )
                    break
