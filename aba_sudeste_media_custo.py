...
from conexao_db import conectar  # ajuste conforme seu módulo real
# importe outras dependências que usar (ex: decimal) se necessário

class AbaSudesteMediaCusto:
    def __init__(self, aba_sudeste, conn):
        self.aba_sudeste = aba_sudeste
        self.conn = conn

        # Cria e empacota o frame principal da aba
        self.frame_tabela = tk.Frame(self.aba_sudeste)
        self.frame_tabela.pack(expand=True, fill='both', padx=10, pady=10)

        # Cria a treeview
        self.tree = ttk.Treeview(self.frame_tabela, columns=(
            'Produto', 'Estoque', 'Custo Estoque', 'Custo Empresa', 'Custo 5%',
            'Custo 10%', 'Custo 15%', 'Custo 20%'
        ...
        try:
            ...
            print('Erro ao popular aba Sudeste:', e)

        # Ajustes de opções da aba
        try:
            self.aba_sudeste.option_add('*TButton*background', '#D3D3D3')
            self.aba_sudeste.option_add('*TButton*foreground', 'black')
            self.aba_sudeste.option_add('*TButton*font', ('Arial', 10))
        except Exception:
            pass

    def _popular(self):
        """Carrega produtos e insere linhas na treeview"""
    ...
    def conectar_banco(self):
        """Retorna uma nova conexão usando seu helper 'conectar'."""
        try:
            conn = conectar()
            return conn
        except Exception as e:
    ...
    def calcular_custo_10(self, custo_base):
        return self.calcular_custo_5(custo_base, percentual=10)

    def calcular_custo_15(self, custo_base):
        return self.calcular_custo_5(custo_base, percentual=15)

    def calcular_custo_20(self, custo_base):
        return self.calcular_custo_5(custo_base, percentual=20)
...
def criar_media_custo_sudeste(aba_sudeste, conn):
    return AbaSudesteMediaCusto(aba_sudeste, conn)
