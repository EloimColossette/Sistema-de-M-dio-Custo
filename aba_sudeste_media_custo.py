...
def calcular_custo_empresa(conn, nome_produto):
    try:
        ...
        # Obtendo o custo empresa cadastrado
        cursor.execute("""
...
def calcular_media_ponderada_total_com_empresa(conn):
    """Calcula a média ponderada do custo do estoque total e o custo empresa ponderado."""
    produtos = buscar_produtos(conn)
    ...
    for produto in produtos:
        ...
        if estoque_float > 0:
            ...
            # Se a média de custo for 0, ignora o custo empresa para este produto
            if media_custo == 0:
                custo_empresa = 0.0
            else:
...
def criar_media_custo_sudeste(aba_sudeste, conn):
    ...
    frame_tabela = tk.Frame(aba_sudeste)
    ...
    aba_sudeste.option_add("*TButton*background", "#D3D3D3")
    aba_sudeste.option_add("*TButton*foreground", "black")
    aba_sudeste.option_add("*TButton*font", ("Arial", 10))
